...
public class WretchStalkerAI : MonoBehaviour
{
    ...
    [Tooltip("How far to try to move away when retreating.")]
    [SerializeField] float retreatStepDistance = 8f;
    ...
    void UpdateStalking()
    {
        ...
        if (_isRetreating)
        {
            ...
            else
            {
                if (Time.time >= _nextDestinationRefreshTime)
                {
                    ...
                    SetDestination(GetRetreatPoint(retreatStepDistance));
                }
                return;
            }
        }
        ...
        else if (dist < stalkMinDistance)
        {
            ...
            {
                _nextDestinationRefreshTime = Time.time + destinationRefreshInterval;
                SetDestination(GetRetreatPoint(stalkMinDistance - dist + 1.0f));
            }
        }
        else
        {
    ...
    void PerformAttack()
    {
        ...
        if (animator != null)
        {
            if (_hasAttackInt)
            {
                ...
                if (attackInt == _lastAttackInt)
                    attackInt = (attackInt + 1) % 3;
