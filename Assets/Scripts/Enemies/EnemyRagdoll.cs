...
public class EnemyRagdoll : MonoBehaviour
{
    ...
    private static readonly System.Collections.Generic.List<EnemyRagdoll> _activeRagdolls = new();
    ...
    public void CreateDetachedRagdoll(Vector3 forceDirection = default, Vector3 forcePoint = default)
    {
        ...
        EnemyRagdoll newRagdoll = ragdollObject.GetComponent<EnemyRagdoll>();
        if (newRagdoll != null)
        {
            ...
            // Mark the detached ragdoll as an "enemy hit" target for impact effects.
            if (!ragdollObject.TryGetComponent<RagdollHitReceiver>(out _))
                ragdollObject.AddComponent<RagdollHitReceiver>();
    ...
    private void SetLiveEnemyColliders(bool enabled)
    {
        foreach (Rigidbody rb in ragdollRigidbodies)
        {
            if (rb != null)
            {
                // Live enemies are kinematic (no ragdoll physics). If a body was previously
                // non-kinematic (e.g., from a copied ragdoll), clear velocities BEFORE
                // switching to kinematic to avoid Unity warnings.
