...
using Treachery.Weapons.Behaviors;
...
public class WeaponInstance_Hitscan : MonoBehaviour, IUpgradeableWeapon, IWeaponBloomProvider, IWeaponEventSource
{
    [Header("Base Stats")]
    ...
    [Header("Fire Behavior (Refactor)")]
    [Tooltip("Use fire strategy implementation (hitscan) instead of internal fire methods.")]
    [SerializeField] bool useFireBehaviorStrategy = true;
    [SerializeField] HitscanFireBehavior hitscanFireBehavior = new HitscanFireBehavior();

    [Header("Presentation (Refactor)")]
    [Tooltip("When a WeaponPresentationController is present (in WeaponContext), use it for ADS camera FOV, weapon offsets and recoil.")]
    [SerializeField] bool usePresentationController = true;
    ...
    event System.Action<int, int> IWeaponEventSource.AmmoChanged
    {
        add => OnAmmoChanged += value;
        remove => OnAmmoChanged -= value;
    }

    event System.Action<string, string> IWeaponEventSource.TierChanged
    {
        add => OnTierChanged += value;
        remove => OnTierChanged -= value;
    }
    ...
    public float PelletSpreadMultiplier => pelletSpreadMultiplier;

    // Public wrappers used during the behavior-preserving refactor migration.
    public float CalculateDamageWithFalloff_Public(float baseDamage, float distance) => CalculateDamageWithFalloff(baseDamage, distance);
    public void SpawnImpactEffect_Public(RaycastHit hit, bool isHeadshot) => SpawnImpactEffect(hit, isHeadshot);
    public void ProcessPelletHit_Public(RaycastHit hit, float pelletDamage) => ProcessPelletHit(hit, pelletDamage);
    public void ProcessBulletHit_Public(RaycastHit hit, float bulletDamage) => ProcessBulletHit(hit, bulletDamage);

    WeaponContext? _weaponContext;

    bool UsingPresentation
    {
        get
        {
            if (!usePresentationController) return false;
            if (!_weaponContext.HasValue) return false;
            return _weaponContext.Value.Presentation != null;
        }
    }

    WeaponPresentationProfile BuildPresentationProfile()
    {
        // Ensure weaponGFX is populated if auto-find is enabled.
        ...
        return new WeaponPresentationProfile(
            supportsADS,
            weaponGFX,
            adsFOV,
            adsTransitionSpeed,
    ...
    {
        _weaponContext = context;

        if (UsingPresentation)
        {
            var profile = BuildPresentationProfile();
            _weaponContext.Value.Presentation.Bind(in profile);
            _weaponContext.Value.Presentation.SetAiming(false);
        }
    }
    ...
    public void Unequip()
    {
        if (UsingPresentation)
        {
            _weaponContext.Value.Presentation.Unbind();
        }
        _weaponContext = null;
    }
    ...
    {
        // Intentionally empty: WeaponInstance_Hitscan still uses MonoBehaviour.Update.
    }
    ...
    public void TryFire()
    {
        ...
        if (UsingPresentation)
        {
            _weaponContext.Value.Presentation.ApplyRecoil(finalRecoilMultiplier);
        }
        else if (weaponRecoil != null)
        {
        ...
        if (useFireBehaviorStrategy)
        {
            hitscanFireBehavior ??= new HitscanFireBehavior();
            hitscanFireBehavior.Fire(this, bloomSpread);
        }
        else
        {
            if (usePelletSystem)
    ...
    private void ProcessPelletHit(RaycastHit hit, float pelletDamage)
    {
        ...
        bool countsAsEnemy;
        DispatchHit(hit, falloffDamage, CurrentBulletForce, Vector3.zero, out countsAsEnemy);
    ...
    private void ProcessBulletHit(RaycastHit hit, float bulletDamage)
    {
        ...
        bool countsAsEnemy;
        DispatchHit(hit, falloffDamage, CurrentBulletForce, hit.normal, out countsAsEnemy);

        // Spawn appropriate impact effect
        SpawnImpactEffect(hit, false);
    }

    private void DispatchHit(RaycastHit hit, float damageAmount, float hitBulletForce, Vector3 legacyHitNormal, out bool countsAsEnemy)
    {
        countsAsEnemy = false;
        if (hit.collider == null) return;

        // New path: target-driven receiver
        IHitReceiver receiver = hit.collider.GetComponent<IHitReceiver>();
        if (receiver == null)
            receiver = hit.collider.GetComponentInParent<IHitReceiver>();

        if (receiver != null)
        {
            var payload = new HitPayload(
                damage: damageAmount,
                bulletForce: hitBulletForce,
                point: hit.point,
                normal: legacyHitNormal,
                hitCollider: hit.collider,
                source: gameObject);

            receiver.ReceiveHit(in payload);
            countsAsEnemy = receiver.CountsAsEnemyHit;
            return;
        }

        // Legacy fallback during migration (no EnemyHealth / HeadshotZone special cases here)
        IDamageable damageable = hit.collider.GetComponent<IDamageable>();
        if (damageable == null)
            damageable = hit.collider.GetComponentInParent<IDamageable>();
        if (damageable != null)
        {
            damageable.TakeDamage(damageAmount, hit.point, legacyHitNormal);
            countsAsEnemy = true;
        }
    }
    ...
    private void SpawnImpactEffect(RaycastHit hit, bool isHeadshot)
    {
        ...
        // Check if we hit an enemy via the new receiver interface.
        bool hitEnemy = false;
        if (hit.collider != null)
        {
            var receiver = hit.collider.GetComponent<IHitReceiver>();
            if (receiver == null)
                receiver = hit.collider.GetComponentInParent<IHitReceiver>();
            if (receiver != null)
                hitEnemy = receiver.CountsAsEnemyHit;

            // Legacy fallback for objects not yet migrated to IHitReceiver.
            if (!hitEnemy)
            {
                hitEnemy = hit.collider.GetComponent<IDamageable>() != null ||
    ...
    public void SetAiming(bool aiming)
    {
        ...
        if (UsingPresentation)
        {
            _weaponContext.Value.Presentation.SetAiming(_isAiming);
        }
    }
    ...
    void UpdateADS()
    {
        // When using presentation controller, it owns camera FOV + weapon offsets.
        if (UsingPresentation)
            return;
